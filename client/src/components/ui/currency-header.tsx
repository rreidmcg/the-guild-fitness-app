...
export function CurrencyHeader() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bg-gradient-to-r from-amber-100/95 via-orange-100/95 to-yellow-100/95 backdrop-blur-sm border-b border-amber-300/50 px-2 sm:px-4 py-1.5 z-40 shadow-lg">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <button
              onClick={handleMusicToggle}
              className="flex items-center hover:bg-amber-300/50 px-1.5 py-0.5 rounded transition-colors"
              title="Toggle Music"
            >
              {isMuted || !isPlaying ? (
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div className="flex items-center space-x-1.5">
              ...
              <span className="font-semibold text-amber-900 text-sm">{(userStats as any)?.currentStreak || 0}</span>
            </div>
            <div className="flex items-center space-x-1.5">
              <Snowflake className="w-3.5 h-3.5 text-blue-600" />
              <span className="font-semibold text-amber-900 text-sm">{(userStats as any)?.streakFreezeCount || 0}</span>
            </div>
            <div className="flex items-center space-x-1.5">
              <Coins className="w-3.5 h-3.5 text-yellow-600" />
              <span className="font-semibold text-amber-900 text-sm">{(userStats as any)?.gold || 0}</span>
            </div>
            <div className="flex items-center space-x-1.5">
              <span className="text-blue-600 text-sm">💎</span>
              <span className="font-semibold text-amber-900 text-sm">{(userStats as any)?.gems || 0}</span>
            </div>
            <div className="flex items-center space-x-1">
              ...
              <button
                ...
                className="flex items-center hover:bg-amber-300/50 px-1.5 py-0.5 rounded transition-colors relative"
                title="Mail"
              >
